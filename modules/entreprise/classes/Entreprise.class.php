...
class Entreprise
{
    ...
    public function setWeight($entreprise_id = 0)
    {
        ...
        // entreprise
        $entreprise = Entreprise::getById($entreprise_id);
    ...
    public function deleteEmail($emailId)
    {
        ...
        if (sizeof(Entreprise::getEmails($this->id))>1)
        {
    ...
    {
        $telephoneFactory = jDao::get('entreprise~telephone');
        if (sizeof(Entreprise::getTelephones($this->id))>1)
        {
    ...
    public static function getMostVisited($n)
    {
        ...
        foreach ($res as $record)
        {
            if ($i<$n)
            {
                $oEntreprise = Entreprise::getById($record->id);
    ...
    {
        $cnx = jDb::getConnection();
        $res = $cnx->query('SELECT * FROM '.$cnx->prefixTable ("entreprise").' WHERE is_publie = 1 ORDER BY date_creation DESC');
        $toEntreprise = array();
        $i=0;
        foreach ($res as $record)
        {
    ...
    public function getCategorieIcon()
    {
        ...
        $oCategorie = Categorie::getById($oSousCategorie->souscategorie_categorieid);
    ...
    public static function search($to_find = null, $hasPagination = true, &$nbRecs = 0, $currentPage = 1, $sortField = "weight", $sortSens = "DESC", $numValPerPage = NB_DATA_PER_PAGE)
    {
        ...
        $query_search = 'SELECT entreprise_id, MATCH (raisonsociale, activite, adresse, region, services, produits, marques, catalogues, souscategories, motscles, telephones) AGAINST ("' . $to_find . '" IN BOOLEAN MODE) AS cpt FROM ' . $cnx->prefixTable("entreprise_search") . ' WHERE MATCH (raisonsociale, activite, adresse, region, services, produits, marques, catalogues, souscategories, motscles, telephones) AGAINST ("' . $to_find . '" IN BOOLEAN MODE) ORDER BY cpt DESC';
        ...
        if (!empty($entreprise_ids)) {
            ...
            foreach ($res_entreprise as $entreprise_row) {
                $results[] = Entreprise::getById($entreprise_row->id);
            }
