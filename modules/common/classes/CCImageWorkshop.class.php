...
jClasses::inc ("common~CCImageWorkshopException") ;

/**
 * CCImageWorkshop class
 *
 * Powerful PHP class using GD library to work easily with images including layer notion (like Photoshop or GIMP).
 * CCImageWorkshop can be used as a layer, a group or a document.
 *
 * @version 1.3.3
...
class CCImageWorkshop
{
    ...
    public function __construct($params = array())
    {
        if (!extension_loaded('gd')) {
            throw new CCImageWorkshopException('PHPImageWorkshop requires the GD extension to be loaded.', CCImageWorkshop::ERROR_GD_NOT_INSTALLED);
        }
    ...
    /**
     * Clone method: use it if you want to reuse an existing CCImageWorkshop object in another variable
    ...
    /**
     * Add an existing CCImageWorkshop sublayer and set it in the stack at a given level
    ...
    /**
     * Add an existing CCImageWorkshop sublayer and set it in the stack at the highest level
    ...
    /**
     * Add an existing CCImageWorkshop sublayer and set it in the stack at level 1
    ...
    public function getResult($backgroundColor = null)
    {
        ...
        $backgroundImage = self::generateImage($this->width, $this->height, $backgroundColor, $opacity);
    ...
    public function flip($type = 'horizontal')
    {
        ...
        $temp = self::generateImage($this->width, $this->height);
    ...
    public function initializeImageFrom($path)
    {
        ...
        if (file_exists($path)) {
            ...
            switch ($mimeContentType) {
                ...
                default:
                    throw new CImageWorkshopException('Not an image file (jpeg/png/gif) at "'.$path.'"', CCImageWorkshop::ERROR_NOT_AN_IMAGE_FILE);
                break;
            }

        } else {
            throw new CImageWorkshopException('No such file found at "'.$path.'"', CCImageWorkshop::ERROR_IMAGE_NOT_FOUND);
        }
    }
    ...
    public function initializeImageWith($image)
    {
        ...
        if (gettype($image) != "resource") {
            throw new CImageWorkshopException("You must give a php image var by using initializeImageWith", CCImageWorkshop::ERROR_PHP_IMAGE_VAR_NOT_USED);
        }
    ...
    public function initializeTextImage($text, $fontPath, $fontSize = 13, $fontColor = "ffffff", $textRotation = 0, $backgroundColor = null)
    {
        ...
        $textDimensions = CCImageWorkshop::getTextBoxDimension($fontSize, $textRotation, $fontPath, $text);
