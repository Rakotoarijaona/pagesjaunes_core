...
class CPhotosUpload
{
    ...
    public function doUpload ()
    {
        ...
        if (in_array (strtolower ($zExt), explode (",", MEDIA_TYPE_IMAGE_EXT)))
        {
            ...
            if (file_exists (PHOTOS_FOLDER . "/" . PHOTOS_THUMBNAIL_FOLDER . "/" . $zFinalName))
            {
                $iExistFileCount = 1 ;
                while (file_exists (PHOTOS_MEDIA_FOLDER . "/" . PHOTOS_THUMBNAIL_FOLDER . "/" . $zNoExtName . $this->zSeparator . $iExistFileCount . "." . $zExt))
                {
                    $iExistFileCount++ ;
                }
            ...
            $oLayerThumbnail->save (PHOTOS_FOLDER . "/" . PHOTOS_THUMBNAIL_FOLDER, $zFinalName, $bCreateFolders, $zBackgroundColor, $iImageQuality) ;
            // thumbnail (must)

            // medium
            $oLayerMedium = ImageWorkshop::initFromPath ($_FILES [$this->zFileName]['tmp_name']) ;
            $oLayerMedium->resizeInPixel (MEDIUM_WIDTH, null, true) ;
            $oLayerMedium->save (PHOTOS_FOLDER . "/" . PHOTOS_MEDIUM_FOLDER, $zFinalName, $bCreateFolders, $zBackgroundColor, $iImageQuality) ;
            // medium

            // big
            $oLayerBig = ImageWorkshop::initFromPath ($_FILES [$this->zFileName]['tmp_name']) ;
            $oLayerBig->resizeInPixel (BIG_WIDTH, null, true) ;
            $oLayerBig->save (PHOTOS_FOLDER . "/" . PHOTOS_BIG_FOLDER, $zFinalName, $bCreateFolders, $zBackgroundColor, $iImageQuality) ;
            // big
        }
        else
        {
            $zPathAndFolder = CCommonTools::createFolder (PHOTOS_FOLDER, 0777, true) ;
